...
export const bundle = () => {
  ...
  const sbomFileName = handleSbom({
    distDirectory: path.dirname(pciFilePath),
    outputDirectory,
    packageTempDirectory: outputDirectoryTemp,
    baseName: `qti3-pci-${safeType}_${version}`,
    packageData,
  });
...
function buildItemXml({ identifier, title, label, prompt, typeIdentifier, dataVersion }) {
  ...
  const promptMarkup = `<div>${xmlEscape(prompt || "")}</div>`;
...
<qti-assessment-item xmlns=\"http://www.imsglobal.org/xsd/imsqtiasi_v3p0\"
  ...
  <qti-item-body>
    <qti-portable-custom-interaction response-identifier=\"RESPONSE\" module=\"pci\" custom-interaction-type-identifier=\"${xmlEscape(
      typeIdentifier
    )}\" data-version=\"${xmlEscape(dataVersion)}\">
      <qti-interaction-modules>
        <qti-interaction-module id=\"pci\" primary-path=\"../resources/pci/index\"></qti-interaction-module>
      </qti-interaction-modules>
      <qti-interaction-markup>${promptMarkup}</qti-interaction-markup>
...
function buildManifestXml({ itemHref, testHref, pciJsHref, pciCssHref, assetHrefs, sbomHref }) {
  const resources = [];
  ...
  // QTI has no resource type for a bill of materials, so it is referenced as
  // plain webcontent when it is included in the package.
  if (sbomHref) {
    resources.push({
      id: "RES-SBOM",
      type: "webcontent",
...
// of the delivery to the customer, and QTI has no slot for one. So it is
// written next to the package per version. Use --include-sbom (or
// config.tspci.sbom.includeInPackage) to also ship it inside the package, and
// --no-sbom to skip it altogether.
