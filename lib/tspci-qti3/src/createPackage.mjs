...
function buildManifestXml({ itemHref, testHref, pciJsHref, pciCssHref, assetHrefs, sbomHref }) {
  ...
  // QTI has no resource type for a bill of materials. Listing it as a file of
  // the PCI resource keeps it with the bundle it describes and avoids an
  // unreferenced resource that manifest driven tooling would drop.
  resources.push({
    id: pciJsId,
    type: "webcontent",
    href: pciJsHref,
    files: [pciJsHref, ...(sbomHref ? [sbomHref] : [])],
    dependencies: [],
  });
...
// survive an import into an authoring system, and that is exactly the party
// that needs it. A copy is left next to the package for your own technical
