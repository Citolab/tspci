...
export const bundle = () => {
  ...
  const propertiesToCopy = ["width", "height", "scoring"];
  ...
  fs.copyFileSync(path.join(PACKAGE_ROOT_PATH, pciFile), path.join(outputDirectoryTemp, "/index.js"));

  // zip the folder
  const zipName = `questify-pci-${name}_${packageData.version}.pci`;
  zipDirectory(outputDirectoryTemp, path.join(outputDirectory, zipName)).then(() =>
    // fs.rmdirSync(outputDirectoryTemp, { recursive: true });
    console.log(`created questify pci zip: ${zipName}`)
  );
};
