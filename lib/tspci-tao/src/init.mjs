...
export const init = async () => {
  ...
  const short = ciMode
    ? defaultShort
    : (await askOption(`Enter a short name for the PCI: (default: ${defaultShort}) `)) || defaultShort;

  const defaultLabel = typeIdentifer.length > 5 ? typeIdentifer.substring(0, 5) : typeIdentifer;
  const label = ciMode
    ? defaultLabel
    : (await askOption(`Enter the label of the PCI: (default: ${defaultLabel}) `)) || defaultLabel;
  const scoreMethodArray = ciMode
    ? ["MATCH_CORRECT", "NONE"]
    : (await selectOptions("Which scoring methods do you want to support?", ["MATCH_CORRECT", "NONE"])).values;
  ...
  // add required runtime dependencies from the packager package to the generated project
  const packagerDeps = {
    ...(packagerPackageData.dependencies || {}),
    ...(packagerPackageData.devDependencies || {}),
  };
  Object.keys(packagerDeps).forEach((key) => {
    packageData.devDependencies[key] = packagerDeps[key];
  });
  ...
  if (isTypeScript && sourceFile.indexOf("implements IMSpci<PropTypes>, TAOpci") === -1) {
    ...
    if (sourceFile.indexOf("implements IMSpci<PropTypes> {") === -1) {
      ...
      if (newSourceFile.indexOf(classEndMarker) === -1) {
        console.error("Cannot inject TAO methods automatically. Please implement TAOpci methods manually.");
      } else {
        newSourceFile = newSourceFile.replace(classEndMarker, `\n${defaultImplementation}\n}\n\nconst factory`);
      }
  ...
  setResponse = function(response) {
    ...
    const classEndMarker = "\n}\n\nconst factory";
    if (newSourceFile.indexOf(classEndMarker) === -1) {
      console.error("Cannot inject TAO methods automatically for JavaScript. Please implement manually.");
    } else {
      newSourceFile = newSourceFile.replace(classEndMarker, `\n${jsImplementation}\n}\n\nconst factory`);
    }
