...
export const init = async () => {
  ...
  const short = (await askOption(
    `Enter a short name for the PCI: (default: ${defaultShort})`
  )) || defaultShort;

  const defaultLabel = typeIdentifer.length > 5 ? typeIdentifer.substring(0, 5) : typeIdentifer;
  const label = (await askOption(
    `Enter the label of the PCI: (default: ${defaultLabel})`
  )) || defaultLabel;
  ...
  packageData.dependencies["@citolab/tspci"] = "^1.0.0";
  ...
  if (sourceFile.indexOf("implements IMSpci<PropTypes>, TAOpci") === -1) {
...
setResponse = (response: QtiVariableJSON) => {
  // used to set the response in tao. E.g. when the item author has set a correct response. And you refresh TAO it will call this method to set the response in the PCI in the score-editor.
      ...
      newSourceFile = newSourceFile.substring(0, lastBracket) + defaultImplementation + newSourceFile.substring(lastBracket);

      fs.writeFileSync(path.join(PACKAGE_ROOT_PATH, packageData.source), newSourceFile, "utf8");
      console.log("TAO interface implemented");
    }
  } else {
