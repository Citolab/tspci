...
export const bundle = () => {
  ...
  const propertiesToReplace = {
    ...
    version: "1.0.0",
    description: "my interaction description",
    label: "label",
    short: "short",
    score: ["NONE"],
  };
  ...
  for (const propertyName in propertiesToReplace) {
    if (propertyName === "typeIdentifier") {
      if (!packageData[propertyName]) {
        ...
        return;
      } else if (!/^[A-Za-z0-9]*$/.test(packageData[propertyName])) {
        console.error(
      ...
      if (!packageData[propertyName])
        console.warn(
          `${propertyName} not specified in ${packageData.name} in package.json using ${propertiesToReplace[propertyName]}`
        );
    }
  }
