  ...
  return {
    ...
    async buildStart() {
      for (const target of targets) {
        ...
        const basePath = hasGlobSyntax(srcPattern)
          ? getGlobBase(srcPattern)
          : srcPattern;
        this.addWatchFile(path.resolve(basePath));
        const matches = await fg(srcPattern, { dot: true, onlyFiles: false });
        matches.forEach((match) => this.addWatchFile(path.resolve(match)));
      }
