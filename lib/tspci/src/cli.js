...
const run = async () => {
  ...
  if (init) {
    ...
    if (options.values.includes("typescript")) {
      copyDir(path.resolve(cliPath, "./../preset/typescript/src"), path.join(destinationFolder,'src'));
    ...
    if (options.values.includes("javascript")) {
      copyDir(path.resolve(cliPath, "./../preset/javascript/src"), path.join(destinationFolder,'src'));
      // point to the javascript file
      const packageJson = path.join(destinationFolder, "package.json");
      const packageJsonContent = fs.readFileSync(packageJson, "utf8");
      let packageJsonContentJson = JSON.parse(packageJsonContent);
