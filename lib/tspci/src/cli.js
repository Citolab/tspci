...
const run = async () => {
  ...
  if (init) {
    ...
    // prompt for name
    const pciName = (await askOption("What your pci identifier (typeIdentifier) (default: myPci)  ")) || "myPci";
    ...
    if (!/^[a-zA-Z0-9]+$/.test(pciName)) {
      console.log(
        chalk.yellow("The identifier of your pci can only contain letters and numbers, stripping special characters")
      );
      pciName = pciName.replace(/[^a-zA-Z0-9]/g, "");
    }
    console.log(chalk.green(`pci: ${pciName}`));
    const description = await askOption("What is the description of your pci? (default: pci created with tspci)") || "pci created with tspci"; 
    ...
    if (args.indexOf("--no-install") === -1) {
      ...
      const npm = spawn("npm", ["install"], { cwd: destinationFolder, shell: true });
      npm.stdout.on("data", (data) => {
        console.log(chalk.green(data.toString()));
        console.log(chalk.green("Done! You can now start the dev server with npm run dev"));
  ...
  if (args.indexOf("add") !== -1) {
    ...
    const modifyPci = () => {
      const scriptFile = path.join(path.join(path.join(path.join(NODE_MODULES, `@citolab`), `tspci-${targetname}`), 'src'), `init.mjs`);
      let init = false;
      ...
      if (!init) {
        console.log(chalk.green(`No init script found for @citolab/tspci-${targetname}`));
      }
    }
    ...
    if (!packageJson.devDependencies || !packageJson.devDependencies[`@citolab/tspci-${targetname}`]) {
      ...
      const npm = spawn("npm", ["install", `@citolab/tspci-${targetname}`, "--save-dev"], { cwd: process.cwd(), shell: true });
      // wait till npm install is done
      npm.stdout.on("data", (data) => {
        console.log(chalk.green(`successfull installed @citolab/tspci-${targetname}`));
  ...
  prodBuild().then(() => {
    if (target) {
      ...
      const targetname = args[targetIndex + 1];
      const scriptFile = path.join(path.join(path.join(path.join(NODE_MODULES, `@citolab`), `tspci-${targetname}`), 'src'), `createPackage.mjs`);
      import(`file:///${scriptFile}`).then((script) => {
        script.bundle();
      });
    }
