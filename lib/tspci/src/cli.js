...
function copyDirAndReplaceInFiles(presetFolder, destinationFolder, projectName, pciName) {
  for (const fileOfDir of fs.readdirSync(presetFolder)) {
    // check if object is a file or a folder
    if (fs.statSync(path.join(presetFolder, fileOfDir)).isDirectory()) {
      copyDirAndReplaceInFiles(
        path.join(presetFolder, fileOfDir),
        path.join(destinationFolder, fileOfDir),
        projectName,
        pciName
      );
    } else {
      const fileExtensionsToCheck = [".js", ".ts", ".tsx", ".json"];
      const filename = fileOfDir;
      const file = path.join(presetFolder, filename);
      ...
      if (fileExtensionsToCheck.includes(fileExtension)) {
        ...
        const kebabCasedProjectName = projectName.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase();
        content = content.replace(/###___PROJECT_NAME___###/g, kebabCasedProjectName);
        content = content.replace(/###___PCI_NAME___###/g, pciName);
        content = content.replace(/###___LABEL___###/g, pciName);
        // get max 5 characters of pciName to create a short name
        const shortName = pciName.substring(0, 5);
        content = content.replace(/###___SHORT___###/g, shortName);
...
const run = async () => {
  ...
  if (help) {
    console.log(`•\t${chalk.blue("--help -h")} list these commands`);
    console.log(`•\t${chalk.blue("--build")} build the project`);
    console.log(`•\t${chalk.blue("--watch")} watch the files`);
    console.log(`•\t${chalk.blue("--target")} build a prod item and bundle for platform`);
    console.log("\n");
    console.log(
      `${chalk.blueBright("tspci init")} creates a new project \n•\t${chalk.blue(
        ...
        "--no-install"
      )} don't install dependencies \n•\t${chalk.blue("--path")} path to create the project in`
    );
    console.log("\n\n");
    return;
  }
  ...
  if (init) {
    ...
    // prompt for name
    const projectName = (await askOption("What is the name of your project? (default: pciProject)  ")) || "pciProject";
    let destinationFolder = path.join(process.cwd(), projectName);
    if (args.find((arg) => arg.startsWith("--path"))) {
      const pathIndex = args.findIndex((arg) => arg.startsWith("--path"));
      destinationFolder = path.join(process.cwd(), path.join(args[pathIndex].split("=")[1], projectName));
    }
    ...
    while (projectName && fs.existsSync(destinationFolder)) {
      console.log(
        chalk.red(
    ...
    let pciName = (await askOption("What is the name of your pci? (default: myPci)")) || "myPci";

    // check if contains other characters than letters and numbers
    if (!/^[a-zA-Z0-9]+$/.test(pciName)) {
      console.log(
        chalk.yellow("The name of your pci can only contain letters and numbers, stripping special characters")
      );
      pciName = pciName.replace(/[^a-zA-Z0-9]/g, "");
    }
    ...
    copyDirAndReplaceInFiles(presetFolder, destinationFolder, projectName, pciName);
    ...
    if (options.values.includes("preact")) {
      ...
      copyDirAndReplaceInFiles(preactPresetFolder, destinationFolder, projectName, pciName);
      // add preact and @citolab/preact-store to the package json
      packageJsonContentJson.dependencies = {
        ...packageJsonContentJson.dependencies,
        preact: "^10.13.2",
        "@citolab/preact-store": "^2.0.9",
      };
    ...
    // run npm instaal in destination folder
    if (args.indexOf("--no-install") === -1) {
      console.log(chalk.green("Installing dependencies"));
      const npm = spawn("npm", ["install"], { cwd: destinationFolder });
      npm.stdout.on("data", (data) => {
        console.log(chalk.green(data.toString()));
        console.log(chalk.green("Done!"));
        console.log(chalk.blue("Starting dev server..."));
        // run npm run dev in destination folder
        const npm = spawn("npm", ["run", "dev"], { cwd: destinationFolder });
        npm.stdout.on("data", (data) => {
