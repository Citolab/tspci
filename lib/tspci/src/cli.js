...
const run = async () => {
  ...
  if (init) {
    ...
    if (args.indexOf("--no-install") === -1) {
      ...
      console.log(chalk.blue(`Navigate to ${destinationFolder} or open this folder (e.g. with vscode).`));
      console.log(chalk.blue(`Start the dev server with: npm run dev`));
      console.log(chalk.blue(`Add support for TAO with: npm run tspci -- add --target tao`));
  ...
  if (args.indexOf("add") !== -1) {
    ...
    const version = args.indexOf("--next") ? "next" : "latest";
