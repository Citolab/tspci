...
function copyDirAndReplaceInFiles(presetFolder, destinationFolder, projectName, pciName) {
  for (const fileOfDir of fs.readdirSync(presetFolder)) {
    ...
    if (fs.statSync(path.join(presetFolder, fileOfDir)).isDirectory()) {
      ...
      const fileExtensionsToCheck = [".js", ".ts", ".json"];
      const filename = fileOfDir;
      const file = path.join(presetFolder, filename);
      ...
      if (fileExtensionsToCheck.includes(fileExtension)) {
        ...
        content = content.replace(/###___PROJECT_NAME___###/g, projectName);
...
const run = async () => {
  ...
  if (init) {
    ...
    // check if arg contain --path='folder'
    ...
    while (fs.existsSync(projectName)) {
      ...
      projectName = (await askOption("What is the name of your project? (default: pciProject)  ")) || "pciProject";
    }
    ...
    const options = await selectOptions("Would you like to add other features?", [
      "tailwind css",
      "preact",
