...
const run = async () => {
  ...
  const targetName = t !== -1 ? `@citolab/tspci-${args[t]}` : tExt !== -1 ? args[tExt] : null;
  ...
  if (init) {
    ...
    if (args.indexOf("--no-install") === -1) {
      ...
      execSync("npm install chokidar@4.0.0 fsevents@latest", { cwd: destinationFolder, stdio: [0, 1, 2] }); // Somehow this to prevent the fsevents.watch is not a function error
