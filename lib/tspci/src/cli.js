...
const ensureRequiredDevDependencies = (packageJson) => {
...
const isMissingTslibError = (event) => {
  const error = event?.error;
  const message = [error?.message, error?.toString?.(), event?.message].filter(Boolean).join("\n");
  return message.includes("Could not find module 'tslib'");
};

const getDevIndexHtmlSourceCandidates = (target = "IMS") => {
  const projectSource = path.join(PACKAGE_ROOT_PATH, "src/index.html");
  const templateSource =
    target === "IMS"
      ? path.join(__dirname, "../public/index.html")
      : path.join(NODE_MODULES, `@citolab/tspci-${target}/public/index.html`);
  return { projectSource, templateSource };
};

const resolveActiveDevIndexHtmlSource = (target = "IMS") => {
  const { projectSource, templateSource } = getDevIndexHtmlSourceCandidates(target);
  return fs.existsSync(projectSource) ? projectSource : templateSource;
};

const copyDevIndexHtmlToDist = (target = "IMS", reason = "initial") => {
  const source = resolveActiveDevIndexHtmlSource(target);
  if (!fs.existsSync(source)) {
    throw new Error(`No index.html template found for dev mode. Expected: ${source}`);
  }
  fs.copyFileSync(source, path.join(PACKAGE_DIST_PATH, "index.html"));
  console.log(`index.html copied to dist (${reason}) from ${source}`);
};

const watchDevIndexHtmlSources = (target = "IMS") => {
  const { projectSource, templateSource } = getDevIndexHtmlSourceCandidates(target);
  const watcher = chokidar.watch([projectSource, templateSource], {
    ignoreInitial: true,
    awaitWriteFinish: {
  ...
  watcher.on("all", (event, changedPath) => {
    try {
      copyDevIndexHtmlToDist(target, `${event}: ${changedPath}`);
    } catch (err) {
      console.error(`Failed to refresh index.html after ${event} on ${changedPath}`, err);
    }
  });
...
const watchBuild = (devServer = false, target = "IMS") => {
  ...
  if (devServer) {
    try {
      copyDevIndexHtmlToDist(target);
      watchDevIndexHtmlSources(target);
    } catch (err) {
      console.error(err);
    }
  }
  loadConfigFile(path.resolve(__dirname, rollupFile), {}).then(async ({ options, warnings }) => {
    ...
    watcher.on("event", (event) => {
      ...
      if (event.code == "ERROR") {
        if (isMissingTslibError(event)) {
          console.log(chalk.red("ERROR: Missing dependency 'tslib'"));
          console.log(chalk.yellow("Install it in your PCI project: npm install --save-dev tslib"));
        }
...
async function selectOptions(prompt, options) {
  const value = await select({
    message: prompt,
    choices: options.map((o) => ({ value: o })),
    default: options[0],
  });
  return { value };
}
...
const run = async () => {
  ...
  if (help) {
    ...
    console.log(
      `•\t${chalk.blueBright("--no-include-sbom")} keep the generated sbom out of the exported package (it is included by default)`
    );
    console.log(`•\t${chalk.blueBright("--no-sbom")} do not generate an sbom for this build`);
    console.log("\n");
    console.log(
  ...
  if (init) {
    ...
    ensureRequiredDevDependencies(packageJsonContentJson);
    ...
    if (args.indexOf("--no-install") === -1) {
      ...
      console.log(`Navigate to ${chalk.bold(destinationFolder)} or open this folder (e.g. with vscode).`);
      console.log(`Start the dev server with: ${chalk.bold("npm run dev")}`);
      console.log(`Add support for TAO with: ${chalk.bold("npm run tspci -- add --target tao")}`);
  ...
  if (args.indexOf("add") !== -1) {
    ...
    const passCiFlag = args.indexOf("--ci") !== -1;
    const packageFilePath = path.join(PACKAGE_ROOT_PATH, "package.json");
    const readPackageJson = () => JSON.parse(fs.readFileSync(packageFilePath, "utf8"));
    ...
    const modifyPci = () => {
      ...
      if (fs.existsSync(scriptFile)) {
        // run init() script
        import(pathToFileURL(scriptFile).href).then((script) => {
          if (script.init) {
            init = true;
    ...
    // add a package script so users can export with a single command
    const targetArg = args[targetIndex + 1];
    if (targetArg) {
      packageJson.scripts = packageJson.scripts || {};
      const packageScriptName = `package:${targetArg}`;
      if (!packageJson.scripts[packageScriptName]) {
        packageJson.scripts[packageScriptName] = `tspci --target ${targetArg}`;
        fs.writeFileSync(packageFilePath, JSON.stringify(packageJson, null, 2), "utf8");
        console.log(chalk.green(`Added script "${packageScriptName}" to package.json`));
      }
    }
