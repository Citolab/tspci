...
const run = async () => {
  ...
  if (init) {
    ...
    const options = await selectOptions("Type of project?", ["preact+tailwind", "typescript", "javascript"]);
    ...
    if (options.values.includes("preact+tailwind")) {
    ...
    if (options.values.includes("typescript")) {
      // keep package.json as is and remove .js
      fs.rmSync(path.join(destinationFolder, "src/index.js"), { force: true });
      fs.rmSync(path.join(destinationFolder, "src/interaction.js"), { force: true });
    } else {
      fs.rmSync(path.join(destinationFolder, "src/index.ts"), { force: true });
      fs.rmSync(path.join(destinationFolder, "src/interaction.ts"), { force: true });
  ...
  if (args.indexOf("add") !== -1) {
    ...
    if (!packageJson.devDependencies || !packageJson.devDependencies[`@citolab/tspci-${targetname}`]) {
      ...
      execSync(`npm install @citolab/tspci-${targetname}@${version} --save-dev`, {
